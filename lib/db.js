...
const AVAIL_LIMITS = { facts: 4, status: 40, label: 30, value: 90 };

export function normaliseAvailability(doc) {
  if (!doc || typeof doc !== 'object') throw new ValidationError('Body must be an object.');
  const status = str(doc.status, AVAIL_LIMITS.status);
  if (!status) throw new ValidationError('The status text (e.g. "Open to work") is required.');
