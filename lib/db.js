...
/* ---------- career timeline (one JSON document: { updated, items[] }) ---------- */
/*
 * Production: site_settings row with key = 'career'. Local dev without a database:
 * public/career.json, also the committed seed and the static fallback until first published.
 */
const CAREER_FILE = path.join(PUBLIC_DIR, 'career.json');
const CAREER_LIMITS = { items: 40, title: 160, company: 120, team: 200, location: 80, bullets: 20, bullet: 400, summary: 1000 };

export function normaliseCareer(doc) {
  if (!doc || typeof doc !== 'object' || !Array.isArray(doc.items)) throw new ValidationError('Body must be an object with an items array.');
  if (doc.items.length > CAREER_LIMITS.items) throw new ValidationError(`At most ${CAREER_LIMITS.items} roles are supported.`);
  const seen = new Set();
  const month = (v) => { const n = parseInt(v, 10); return n >= 1 && n <= 12 ? n : null; };
  const year = (v) => { const n = parseInt(v, 10); return n >= 1970 && n <= 2100 ? n : null; };
  const items = doc.items.map((j, i) => {
    if (!j || typeof j !== 'object') throw new ValidationError(`Role ${i + 1} is not an object.`);
    const title = str(j.title, CAREER_LIMITS.title), company = str(j.company, CAREER_LIMITS.company);
    if (!title) throw new ValidationError(`Role ${i + 1} needs a title.`);
    if (!company) throw new ValidationError(`"${title}" needs a company.`);
    const sm = month(j.sm), sy = year(j.sy);
    if (!sm || !sy) throw new ValidationError(`"${title}" needs a valid start month and year.`);
    const current = !!j.current;
    let em = null, ey = null;
    if (!current) {
      em = month(j.em); ey = year(j.ey);
      if (!em || !ey) throw new ValidationError(`"${title}" needs an end month and year, or tick "I currently work here".`);
      if (ey * 12 + em < sy * 12 + sm) throw new ValidationError(`"${title}": the end date is before the start date.`);
    }
    const bullets = (Array.isArray(j.bullets) ? j.bullets : String(j.bullets || '').split(/\r?\n/))
      .map((b) => str(b, CAREER_LIMITS.bullet)).filter(Boolean);
    if (bullets.length > CAREER_LIMITS.bullets) throw new ValidationError(`"${title}": at most ${CAREER_LIMITS.bullets} highlights.`);
    let id = slug(j.id) || slug(company) || `job-${i + 1}`;
    while (seen.has(id)) id = id.replace(/(-\d+)?$/, (m) => `-${(parseInt(m.slice(1), 10) || 1) + 1}`);
    seen.add(id);
    return { id, title, company, team: str(j.team, CAREER_LIMITS.team), location: str(j.location, CAREER_LIMITS.location), sm, sy, current, em, ey, bullets, summary: str(j.summary, CAREER_LIMITS.summary) };
  });
  return { items };
}
...
export async function readCareer() {
  if (useFile()) {
    try { return { ...JSON.parse(await readFile(CAREER_FILE, 'utf8')), source: 'file' }; }
    catch (e) { if (e.code === 'ENOENT') return null; throw e; }
  }
  await ensureSchema();
  const rows = await db()`select value from site_settings where key = 'career'`;
  return rows[0] ? { ...rows[0].value, source: 'db' } : null;
}

export async function replaceCareer(input) {
  const doc = { updated: new Date().toISOString().slice(0, 10), ...normaliseCareer(input) };
  if (useFile()) { await writeFile(CAREER_FILE, JSON.stringify(doc, null, 2) + '\n', 'utf8'); return { ...doc, source: 'file' }; }
  await ensureSchema();
  await db()`insert into site_settings (key, value) values ('career', ${JSON.stringify(doc)}::jsonb)
    on conflict (key) do update set value = excluded.value`;
  return { ...doc, source: 'db' };
}
